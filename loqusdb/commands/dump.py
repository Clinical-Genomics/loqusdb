...
@click.option(
    "-f", "--filename", help="If custom named file is to be used", type=click.Path(exists=False)
)
@click.pass_context
def dump(ctx, filename):
    ...
    if os.path.isfile(filename):
        LOG.warning(
            "File {} already exists. Please remove file or change name with '--filename'".format(
                filename
            )
        )
        ctx.abort()
